...
async def job_alerts(keywords):
    job_datas = []
    if " " in keywords:
        keywords = keywords.replace(" ", "%20")
    for page in range(0, 20, 10):
        rss_url = f"https://www.upwork.com/ab/feed/jobs/rss?paging=0-10&q={keywords}&sort=recency&api_params=1&securityToken=9036a5d7ac251defd2cbbdb953be6517dccc8dc88a16442dbe27b213db19c07588315c71fa8e894087e88e44384a7743bf0a75944eecda66f0909750157b2e4d&userUid=1368080374926864384&orgUid=1368080374926864385https://www.upwork.com/ab/feed/jobs/rss?paging=0-10&q={keywords}&sort=recency&api_params=1&securityToken=9036a5d7ac251defd2cbbdb953be6517dccc8dc88a16442dbe27b213db19c07588315c71fa8e894087e88e44384a7743bf0a75944eecda66f0909750157b2e4d&userUid=1368080374926864384&orgUid=1368080374926864385"
        ...
        for job in jobs:
            for idx in range(10):
                datas = {
                    ...
                    "post_date": [
                        " ".join(j.text.strip().split()[:-2])
                        for j in job.select("pubDate")
                    ][idx],
                    "post_time": [
                        j.text.strip().split()[-2] for j in job.select("pubDate")
                    ][idx],
                }
